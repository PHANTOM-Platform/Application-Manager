	...
	for i do
		if [ "$nuevo" = true ]; then
			if [ "$last" = "-s" ] || [ "$last" = "-S" ]; then
				...
				mytoken=$i;
				nuevo=false;
			elif [ "$last" = "-device" ] || [ "$last" = "-DEVICE" ]; then
				device=$i;
				nuevo=false;	 
			elif [ "$i" = "-h" ] || [ "$i" = "-H" ]; then
				...
				echo -e "${yellow}      device for which we query data [ -device 1234 ] ${reset}";   # ../web/exampleh.json  json_file 
				...
				echo -e "\n${yellow} Example of use:\n     ${app} -t f7vglñerghpq3ghwoghw -device \"laptop-i7/\";  ${reset}\n"; 
				exit 0;
			elif [ "$last" != "" ]; then
