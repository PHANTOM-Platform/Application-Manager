...
# 3. ################## DELETE DATABASE ###################################
	...
	read -p $'Press [Enter] key to run the script for \033[1;37mDELETING\033[1;34m the APP MANAGER'; echo -ne "${NO_COLOUR}";
	bash delete_db.sh -s ${server} -port ${appmanager_port} ;
# 4. ################## CREATE A NEW DATABASE ###################################
	echo -e "\n${LIGHT_BLUE}";
	echo "bash create_db.sh -s ${server} -port ${appmanager_port} ;";
	read -p $'Press [Enter] key to \033[1;37mCREATE\033[1;34m a new empty APP MANAGER \033[1;37mDB\033[1;34m'; echo -ne "${NO_COLOUR}";
	bash create_db.sh -s ${server} -port ${appmanager_port} ;
# 5. ################## REGISTER A NEW USER ###################################
	echo -e "\n${LIGHT_BLUE}";
	echo "bash register_user.sh -e bob@example.com -pw 1234 -s ${server} -port ${appmanager_port} ;";
	read -p $'Press [Enter] key to run the script for \033[1;37mREGISTER\033[1;34m a new user'; echo -ne "${NO_COLOUR}";	
	bash register_user.sh -e bob@example.com -pw 1234 -s ${server} -port ${appmanager_port} ;
