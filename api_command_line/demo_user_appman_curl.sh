...
# 10. ################## TEST ACCESS WITHOUT A TOKEN, access must be rejected UNAUTHORIZED:401 ####################
	echo -e "\n${LIGHT_BLUE}";
	echo "curl -H \"Content-Type: multipart/form-data\" -XPOST -F \"UploadJSON=@../web/example_manager_task.json\" http://${server}:${appmanager_port}/register_task" ;
	read -p $'Press [Enter] key to \033[1;37mUPLOAD\033[1;34m a file \033[1;37mWITHOUT TOKEN\033[1;34m'; echo -ne "${NO_COLOUR}";
	curl -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web/example_manager_task.json" http://${server}:${appmanager_port}/register_task ;
# 11. ################## TEST OF UPLOADING A JSON FILE WITH A NOT VALID TOKEN, access must be rejected UNAUTHORIZED:401 ######## 
	echo -e "\n${LIGHT_BLUE}";
	echo "curl -s -H \"Authorization: OAuth 12345678\" -H \"Content-Type: multipart/form-data\" -XPOST -F \"UploadJSON=@../web/example_manager_task.json\" http://${server}:${appmanager_port}/register_task";
	...
	curl -s -H "Authorization: OAuth 12345678" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web/example_manager_task.json" http://${server}:${appmanager_port}/register_task?DestFileName=main.c\&'Path=mypath/';
# 12. ################## TEST OF UPLOADING A FILE WITH A NOT VALID TOKEN, access must be rejected UNAUTHORIZED:401 ######## 
	echo -e "\n${LIGHT_BLUE}";
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -H \"Content-Type: multipart/form-data\" -XPOST -F \"UploadJSON=@../web/example_manager_task.json\" http://${server}:${appmanager_port}/register_task ";
	...
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web/example_manager_task.json" http://${server}:${appmanager_port}/register_task ;
...
# 17. ######## TEST OF DOWNLOADING METADATA WITH A VALID TOKEN for all tasks in a single project, access must be accepted : 200 ########
	echo -e "\n${LIGHT_BLUE}";
	echo "Now only QUERY on the filepath";
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -XGET http://${server}:${appmanager_port}/query_metadata?project=demo_hpc ";
	read -p $'Press [Enter] key to \033[1;37mRETRIEVE METADATA\033[1;34m with \033[1;37mVALID TOKEN\033[1;34m'; echo -e "${NO_COLOUR}" ;
	curl -s -H "Authorization: OAuth ${mytoken}" -XGET http://${server}:${appmanager_port}/query_metadata?project=demo_hpc  ;
# 18. ################## TEST OF DOWNLOADING METADATA WITH A VALID TOKEN and USER DEFINED QUERY ##########################
	...
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -H \"Content-Type: multipart/form-data\" -XGET http://${server}:${appmanager_port}/es_query_metadata?QueryBody=\"\\{\\\"query\\\":\\{\\\"bool\\\":\\{\\\"must\\\":\\[\\{\\\"match_phrase\\\":\\{\\\"project\\\":\\\"demo_hpc\\\"\\}\\},\\{\\\"term\\\":\\{\\\"project_length\\\":8\\}\\}]\\}\\}\\}\"";
	
	read -p $'Press [Enter] key to \033[1;37mRETRIEVE METADATA\033[1;34m with \033[1;37mVALID TOKEN\033[1;34m'; echo -ne "${NO_COLOUR}" ;
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XGET http://${server}:${appmanager_port}/es_query_metadata?QueryBody="\{\"query\":\{\"bool\":\{\"must\":\[\{\"match_phrase\":\{\"project\":\"demo_hpc\"\}\},\{\"term\":\{\"project_length\":8\}\}\]\}\}\}";
