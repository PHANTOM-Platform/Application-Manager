...
# 18. ################## TEST USER DEFINED QUERY ##########################
	...
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -H \"Content-Type: multipart/form-data\" -XGET http://${server}:${appmanager_port}/es_query_exec?QueryBody=\"\\{\\\"query\\\":\\{\\\"bool\\\":\\{\\\"must\\\":\\[\\{\\\"match\\\":\\{\\\"app\\\":\\\"HPC%20USE%20CASE\\\"\\}\\}\\]\\}\\}\\}\"";
	read -p $'Press [Enter] key to \033[1;37mRETRIEVE execution\033[1;34m with \033[1;37mVALID TOKEN\033[1;34m'; echo -ne "${NO_COLOUR}" ;
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XGET http://${server}:${appmanager_port}/es_query_exec?QueryBody="\{\"query\":\{\"bool\":\{\"must\":\[\{\"match\":\{\"app\":\"HPC%20USE%20CASE\"\}\}\]\}\}\}";
