...
# 12. ################## TEST OF UPLOADING A FILE WITH A VALID TOKEN,  ######## 
	echo -e "\n${LIGHT_BLUE}";
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -H \"Content-Type: multipart/form-data\" -XPOST -F \"UploadJSON=@../web/example_manager_task.json\" http://${server}:${appmanager_port}/update_project_tasks;";
	...
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web/example_manager_task.json" http://${server}:${appmanager_port}/update_project_tasks;
...
# 17. ######## TEST OF DOWNLOADING METADATA WITH A VALID TOKEN for all tasks in a single project, access must be accepted : 200 ########
	echo -e "\n${LIGHT_BLUE}";
	echo "QUERY on project";
	echo "curl -s -H \"Authorization: OAuth \${mytoken}\" -XGET http://${server}:${appmanager_port}/query_task?project=demo_hpc\&pretty=true;";
	read -p $'Press [Enter] key to \033[1;37mRETRIEVE METADATA\033[1;34m with \033[1;37mVALID TOKEN\033[1;34m'; echo -e "${NO_COLOUR}" ;
	curl -s -H "Authorization: OAuth ${mytoken}" -XGET http://${server}:${appmanager_port}/query_task?project=demo_hpc\&pretty=true ;
