...
#. ############################# UPLOAD A FILE ###############################
	echo -e "\n\n******************\nREGISTER PROJECT 1" 
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web-appmanager/example_manager_task.json" http://${server}:${appmanager_port}/register_new_project ;
	...
	echo -e "\n\n******************\nREGISTER PROJECT 2"
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web-appmanager/example_manager_task3.json" http://${server}:${appmanager_port}/update_project_tasks ;
	...
	curl -s -H "Authorization: OAuth ${mytoken}" -H "Content-Type: multipart/form-data" -XPOST -F "UploadJSON=@../web-appmanager/example_manager_task2.json" http://${server}:${appmanager_port}/update_project_tasks ;
