...
public class ws_susc_appmanager extends WebSocketClient {
	public ws_susc_appmanager( URI serverURI ) {
		super( serverURI );
	} 
	...
	public static void main( String[] args ) throws URISyntaxException {
		...
		try {
			ws_client = new ws_susc_appmanager( new URI( serverlocation ));  // We create a WS connection
		...
		try {
			...
			ws_client.send("{\"user\":\"alice@abc.com\" , \"device\":\"HLRS Raspberry pi3\"}"); //user is for debugging purposes, it may help to find lost connections
			ws_client.send("{\"user\":\"alice@abc.com\" , \"execution\":\"HPC USE CASE\"}"); //user is for debugging purposes, it may help to find lost connections
